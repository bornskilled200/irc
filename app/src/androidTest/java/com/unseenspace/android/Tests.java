...
public final class Tests {
    ...
    /**
     * Rotate the device given it's activity and requested Orientation.
     *
     ...
     * @param <T> the activity given will be the activity returned
    ...
    public static StackTraceElement findTestClassTraceElement(StackTraceElement[] trace) {
        for (int i = trace.length - 1; i >= 0; i--) {
            ...
            try {
                ...
                Method declaredMethod = aClass.getDeclaredMethod(element.getMethodName());
                for (Annotation annotation : declaredMethod.getDeclaredAnnotations()) {
                    if (annotation.annotationType().equals(Test.class))
                        return element;
                }
            } catch (NoSuchMethodException e) {
                Log.v("Themes", "ignoring", e);
    ...
    public static Activity getCurrentActivity() {
        ...
        InstrumentationRegistry.getInstrumentation().runOnMainSync(new Runnable() {
            public void run() {
                ...
                Iterator<Activity> resumedActivities = instance.getActivitiesInStage(Stage.RESUMED).iterator();
