...
public class Server implements Runnable
{
    ...
    {
        private ArrayList<Server> channelMembers = new ArrayList<Server>();
        private String topic;
        protected String name;
        
        public void sendNot(Server not, String toSend)
        {
            synchronized (mutex)
            {
                for (Server con : channelMembers)
                {
                    if (con != not)
                        con.send(toSend);
                }
    ...
    public static Map<String, Server> connectionMap = new HashMap<String, Server>();
    ...
    public static void main(String[] args) throws Throwable
    {
        if (args.length == 0)
        {
            System.out.println("Usage: java jw.jircs.Server <servername>");
            return;
        }
    ...
    /**
     *
     * @param serverName irc ip/hostname
     */
    public static Future<Socket> one(final String serverName) throws Throwable
    {
        RunnableFuture<Socket> runnable = new RunnableFuture<Socket>() {
            public ServerSocket serverSocket = new ServerSocket(6667);
            private boolean canceled = false;
            public boolean done = false;
            private ArrayBlockingQueue<Socket> sockets = new ArrayBlockingQueue<>(1);
            ...
            @Override
            public Socket get(long timeout, @NonNull TimeUnit unit) throws InterruptedException, ExecutionException, TimeoutException {
                return sockets.poll(timeout, unit);
            }
            ...
            public void run() {
                try {
                    ...
                    if (isCancelled())
                        return;
                    sockets.add(socket);
                    Server jircs = new Server(socket);
                    jircs.run();
                } catch (IOException e) {
                    e.printStackTrace();
                } finally {
    ...
    public enum Command
    {
        ...
        {
            @Override
            public void run(Server con, String prefix, String[] arguments)
                    throws Exception
            {
                if (con.nick == null)
            ...
            private void doSelfSwitchNick(Server con, String nick)
            {
                synchronized (mutex)
                {
            ...
            private void doFirstTimeNick(Server con, String nick)
        ...
        JOIN(1, 2)
        {
            ...
            public void doJoin(Server con, String channelName)
            {
                if (!channelName.startsWith("#"))
                {
                    con
                ...
                synchronized (mutex)
                {
                    ...
                    if (channel.topic != null)
                        ...
                        con.sendGlobal("331 " + con.nick + " " + channel.name
                                + " :No topic is set");
                    for (Server channelMember : channel.channelMembers)
                    {// 353,366
        ...
        USERHOST(1, 5)
        {
                    ...
                    throws Exception
            {
                ...
                {
                    Server user = connectionMap.get(s);
                    if (user != null)
        ...
        public abstract void run(Server con, String prefix,
